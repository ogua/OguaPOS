...
class PosComponent extends Component implements HasForms
{
    ...
    public function checkgiftcard()
    {
        ...
        $cards = Giftcard::where('is_active', true)->where('expiry_date', '>=', date('Y-m-d'))->where('card_no', $card)->first();
        ...
        if ($cards) {
            ...
            if ($paying > $bal) {
                ...
                Notification::make()->title('Gift card added successfully!')->success()->send();

                $this->gift_card_id = $cards->id;
            }
        } else {
            Notification::make()->title('Gift Card expired or dont exist!')->warning()->send();
    ...
    public function create()
    {
        ...
        if ($this->paidby == 'CREDIT CARD') {
            ...
            Notification::make()->title('Paypal Checkout coming soon!')->warning()->send();
    ...
    function savecode($sale)
    {
        ...
        foreach (collect($sale->saleitem) as $row) {
            ...
            if ($product_type == 'Single') {
                $stock = Product_Warehouse_Inventory::where('product_id', $product->id)->where('warehouse_id', $warehouse)->first();
                ...
                $stock = Product_Warehouse_Inventory::where('product_id', $product->id)->where('warehouse_id', $warehouse)->where('variant_id', $row->variant_id)->first();
    ...
    function cashregistercode()
    {
        ...
        $salesbrandSummary = DB::table('sales')
            ...
            ->where('sales.cash_register_id', $this->cash_register->id)
            ->select(DB::raw('COALESCE(brands.name, "-") as brand_name'), DB::raw('SUM(sales_items.qty) as total_qty'), DB::raw('SUM(sales_items.total) as brand_total'))
            ->groupBy('brands.name')
            ->get();

        // Convert to associative array for easier handling in the view
        $salesSummaryArray = $salesbrandSummary
    ...
    public function addtoitems($key, $product, $type)
    {
        ...
        $promo = $product
            ...
            ->when($product_type == 'Variation', function ($query) use ($state, $warehouse_id) {
                return $query->where('warehouse_id', $warehouse_id)->where('variant_id', $state[1]);
            })
        ...
        if ($product_type == 'Single') {
            $product_variant_data = $product->inventory()->where('warehouse_id', $warehouse_id)->first();

            $stock = (int) $product_variant_data->qty;

            $px = $product_variant_data->selling_price;
        } elseif ($product_type == 'Variation') {
            $product_variant_data = $product->inventory()->where('variant_id', $state[1])->first();
        ...
        return $form
            ->schema([
                Forms\Components\Section::make('')
                    ->description('')
                    ->schema([
                        Forms\Components\DatePicker::make('transaction_date')->placeholder('Transaction date')->label('')->default(date('Y-m-d'))->required(),
                        Forms\Components\TextInput::make('reference_number')->label('')->placeholder('Reference number') //->default(request()->routeIs('filament.admin.resources.sales.create'))
                            ->maxLength(255),
                        Forms\Components\Select::make('warehouse_id')->label('')->required()->options(Warehouse::pluck('name', 'id'))->live()->default(1)->preload()->searchable(),
                        ...
                        Forms\Components\Select::make('scan_code')
                            ...
                            ->options(function (): array {
                                $products = [];
                                // Load Single type products
                                $products = Product::select('id', 'product_name', 'product_code', 'product_image')->where('active', true)->where('product_type', 'Single')->get()->toArray(); // Convert to array

                                // Load Variation type products
                                $products_variation_list = Product::select('id', 'product_name', 'product_code', 'product_image', 'product_expiry_date')->where('active', true)->where('product_type', 'Variation')->get();
                            ...
                            ->afterStateUpdated(function (Forms\Set $set, Forms\Get $get, ?string $state) {
                                ...
                                $promo = $product
                                    ...
                                    ->when($product_type == 'Variation', function ($query) use ($state, $get) {
                                        return $query->where('warehouse_id', $get('warehouse_id'))->where('variant_id', $state[1]);
                                    })
                                ...
                                if ($product_type == 'Single') {
                                    $product_variant_data = $product->inventory()->where('warehouse_id', $get('warehouse_id'))->first();

                                    $stock = $product_variant_data->qty;

                                    $px = $product_variant_data->selling_price;
                                } elseif ($product_type == 'Variation') {
                                    $product_variant_data = $product->inventory()->where('variant_id', $state[1])->where('warehouse_id', $get('warehouse_id'))->first();
                        ...
                        TableRepeater::make('items')
                            ...
                            ->schema([
                                ...
                                Forms\Components\Hidden::make('stock')->dehydrated(false),
                                Forms\Components\Hidden::make('defaultprice')->dehydrated(false),
                                Forms\Components\Hidden::make('default_unit_id')->dehydrated(false),
                                Forms\Components\Hidden::make('defaultqty')->dehydrated(false),
                                Forms\Components\Hidden::make('currentqty')->dehydrated(false),
                                ...
                                Forms\Components\TextInput::make('product_name')->columnSpan(2)->readOnly(),

                                Forms\Components\Select::make('sale_unit_id')
                                    ->options(function ($get) {
                                        return Productunit::where('base_unit', $get('default_unit_id'))->orWhere('id', $get('default_unit_id'))->pluck('name', 'id');
                                    })
                                    ->preload()
                                    ->searchable()
                                    ->label('')
                                    ...
                                    ->afterStateUpdated(function ($state, $get, $set) {
                                        ...
                                        $unit = Productunit::where('id', $nnewstate)->first();
                                ...
                                Forms\Components\TextInput::make('display_price')->type('number')->disabled()->label('Price')->required(),
                        ...
                        Forms\Components\Section::make('')
                            ->schema([
                                Forms\Components\Placeholder::make('item')
                                    ->columnSpanFull()
                                    ->content(function ($get, $set) {
                                        $tot = collect($get('items'))->count();

                                        $totqty = collect($get('items'))->pluck('qty')->sum();
                                ...
                                Forms\Components\Hidden::make('coupon_type')->dehydrated(false),
                                ...
                                Forms\Components\TextInput::make('coupon_discount')
                                    ...
                                    ->suffixAction(
                                        Action::make('coupon')
                                            ...
                                            ->form([Forms\Components\TextInput::make('coupon_value')->label('Enter code')->required()])
                                            ->action(function (array $data, Forms\Set $set, Forms\Get $get) {
                                                $coupon = Coupon::where('code', $data['coupon_value'])->where('is_active', true)->first();

                                                $todate = new \DateTime(); // Current date
                                                $expiry_date = new \DateTime($coupon->expiry_date);

                                                if (!$coupon) {
                                                    Notification::make()->title('Invalid coupon code!')->warning()->send();
                                                } else {
                                                    $px = $coupon->amount;

                                                    if ($todate > $expiry_date) {
                                                        Notification::make()->title('This Coupon has expired!')->warning()->send();
                                                    } elseif ($coupon->qty <= $coupon->used) {
                                                        Notification::make()->title('This Coupon is no longer available!')->warning()->send();
                                ...
                                Forms\Components\Hidden::make('order_tax')->default(0),
                                ...
                                Forms\Components\TextInput::make('order_tax_rate')
                                    ...
                                    ->suffixAction(
                                        Action::make('Tax')
                                            ...
                                            ->form([Forms\Components\Select::make('order_tax')->label('')->options(Taxrates::pluck('name', 'id'))->searchable()->required()])
                                            ->action(function (array $data, Forms\Set $set, Forms\Get $get) {
                                ...
                                Forms\Components\Hidden::make('order_discount_type')->default(''),
                        ...
                        Forms\Components\Hidden::make('item')->default(0),
                        Forms\Components\Hidden::make('total_qty')->default(0),
                        Forms\Components\Hidden::make('currency_id'),
                        Forms\Components\Hidden::make('coupon_id'),
                        Forms\Components\Hidden::make('paid_amount')->default(0),
                        Forms\Components\Hidden::make('total_discount')->default(0),
