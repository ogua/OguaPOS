<div>
<div class="rounded w-full shadow-md top-0 z-50">
...
<section class="grid grid-cols-3 lg:grid-cols-3 md:grid-cols-2 gap-x-3 mb-20">
...
<x-filament::loading-indicator wire:loading wire:target="addtoitems('{{ $key }}','{{ $product['id'] }}', 'pass')" class="h-10 w-10 absolute m-auto" />
...
<section class="grid grid-cols-3 lg:grid-cols-3 md:grid-cols-2 gap-x-3 mt-40 mb-20">

@foreach ($allproducts as $key => $product)
<!--   ✅ Product card 1 - Starts Here 👇 -->
<div class="bg-white shadow-md rounded-xl duration-500 hover:scale-105 hover:shadow-xl relative">

<a href="#" wire:click.prevent="addtoitems('{{ $key }}','{{ $product['id'] }}', 'check')">

<x-filament::loading-indicator wire:loading wire:target="addtoitems('{{ $key }}','{{ $product['id'] }}', 'check')" class="h-10 w-10 absolute m-auto " />
...
<x-filament::button color="warning" icon="heroicon-o-currency-dollar" wire:click="openpaywithcashmodal('PAYPAL')">
PAYPAL
</x-filament::button>
